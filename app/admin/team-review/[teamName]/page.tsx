...
export default function TeamReviewPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchTeamMedia = async () => {
      try {
        const response = await fetch(`/api/team-items?team=${encodeURIComponent(params.teamName)}`);
        if (!response.ok) {
    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-3xl font-bold">{params.teamName} Media</h1>
        </div>
            ...
            {mediaItems.map((item) => (
              ...
              >
                {item.type === 'photo' ? (
                  <Image
                    src={item.url}
                    alt={`${params.teamName} ${item.type} ${item.number}`}
                    fill
